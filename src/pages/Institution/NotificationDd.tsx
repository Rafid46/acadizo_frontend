      ...
      {matchedNotices?.length === 0 ? (
        ...
        <div>
          {matchedNotices?.map((item: any) => (
            <article key={item?.noticeId} className="border-t border-gray-200">
              <div className="rounded-[10px] flex justify-between p-2 sm:p-6 items-start">
                <div className="mt-5">
                  <h3 className="mt-0.5 text-sm text-gray-600 mb-2">
                    {item?.title}:
                  </h3>
                  <h3 className="mt-0.5 text-lg font-medium text-gray-600 bg-gray-200 p-2 rounded-lg w-fit ">
                    {item?.description}
                  </h3>
